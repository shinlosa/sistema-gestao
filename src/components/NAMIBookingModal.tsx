...
export function NAMIBookingModal({ 
  ...
  useEffect(() => {
    if (editingBooking) {
      // Editando reserva existente - preencher com dados da reserva
  ...
  const getOccupiedSlots = (date: Date) => {
    return existingBookings
      .filter(booking => 
        ...
        // Excluir a reserva que está sendo editada para permitir alteração de horários
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[95vh] overflow-y-auto w-[95vw]">
        <DialogHeader>
          ...
          <DialogDescription>
            Sala {room?.number} - Capacidade: {room?.capacity} pessoas
          </DialogDescription>
          {editingBooking && (
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            ...
            {/* Lado direito - Calendário e horários */}
            <div className="space-y-4">
              <div>
